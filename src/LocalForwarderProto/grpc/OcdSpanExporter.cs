...
namespace LocalForwarderProto.grpc
{
    public class OcdSpanExporter : Export.ExportBase
    {
        ...
        public OcdSpanExporter()
        {
            ikey = TelemetryConfiguration.Active.InstrumentationKey;
            telemetryClient = new TelemetryClient();
            telemetryClient.Context.GetInternalContext().SdkVersion = "occs:0.0.1-alpha0";
        }
        ...
        public override async Task ExportSpan(IAsyncStreamReader<ExportSpanRequest> requestStream, IServerStreamWriter<ExportSpanResponse> responseStream, ServerCallContext context)
        {
            await requestStream.ForEachAsync(spanRequest =>
            {
                ...
                {
                    Console.WriteLine("start tracking " + span.Name);
                    try
                    {
                        TrackSpan(span);
                        Console.WriteLine("tracked " + span.Name);
                    }
                    catch (Exception e)
                    {
                        Console.WriteLine(e);
